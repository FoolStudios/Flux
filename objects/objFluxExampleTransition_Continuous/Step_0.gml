...
if(keyboard_check_pressed(vk_space)) {
	...
	var _seq = flux_create(seqFadeCompleteExample);
	...
	_seq.addEvent(26, function(_caller, _args) {
		// We want to Prepare the Room For Fade In, we are not copying events but we do want to preserve position
		flux_room_goto(_args[0], -1, false, true);
	}, self, [_target]);
